...
async def export_query_report(req: QueryRequest, db: AsyncSession = Depends(get_db)):
    ...
    report = {
        "report_type": "SOP-Guard Query Report",
    ...
    return JSONResponse(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="sop-guard-report-'
...
async def generate_report(req: QueryRequest, db: AsyncSession = Depends(get_db)):
    ...
    report = {
        "header": {
            "title": "SOP-Guard Clinical Query Report",
            "system": "SOP-Guard v1.0.0",
        ...
        "footer": {
            "note": "This report was generated by SOP-Guard, a research prototype for clinical SOP question-answering. Always verify information against official hospital SOPs before clinical use.",
        },
    }
