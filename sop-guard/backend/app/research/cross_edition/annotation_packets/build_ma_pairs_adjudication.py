"""
One-off script: builds the adjudication workbook for the items where
Annotator K and Annotator L disagreed across the two Massachusetts
confirmatory pairs (v2025.1->v2026.1, v2026.1->v2026.2). Mirrors
build_new_pairs_adjudication.py exactly, relettered for K/L.
...
Run:
    ...
    python -m app.research.cross_edition.annotation_packets.build_ma_pairs_adjudication
"""
import sys
...
COLUMNS = [
    ...
    ("Annotator K said", 25, "vote_K"),
    ("Annotator L said", 25, "vote_L"),
...
INSTRUCTIONS = [
    ...
    (None, "Two people (K and L) independently labelled the same two "
           "Massachusetts 60-item packets (v2025.1->v2026.1 and "
           "v2026.1->v2026.2). On these items, their answers disagreed."),
    ("What to do", None),
...
def build_instructions_sheet(wb: Workbook, n: int):
    ws = wb.active
    ws.title = "READ ME FIRST"
    ws.sheet_view.showGridLines = False
    ws.column_dimensions["A"].width = 100
    r = 1
    c = ws.cell(row=r, column=1, value=f"Massachusetts pairs adjudication - {n} disputed items (K/L)")
...
def build_data_sheet(wb: Workbook, disputes: list[tuple[str, str]], raw_k: dict, raw_l: dict):
    """disputes: list of (pair_sheet_title, sample_id)."""
    ws = wb.create_sheet(title=f"Adjudicate these {len(disputes)}")
    ws.sheet_view.showGridLines = False
    ws.freeze_panes = "A2"
    ...
    for pair, sid in disputes:
        ...
        values = {
            ...
            "vote_K": sanitize(vote_text(raw_k[pair][sid])),
            "vote_L": sanitize(vote_text(raw_l[pair][sid])),
...
if __name__ == "__main__":
    ...
    for pair in PAIR_SLUGS:
        ak = {sid: _norm_answer(v["correspondence"]) for sid, v in raw_k[pair].items()}
        al = {sid: _norm_answer(v["correspondence"]) for sid, v in raw_l[pair].items()}
        for sid in sorted(ak):
            if ak.get(sid) != al.get(sid):
                disputes.append((pair, sid))
    print(f"disputed items: {disputes}")

    wb = Workbook()
    build_instructions_sheet(wb, len(disputes))
    n = build_data_sheet(wb, disputes, raw_k, raw_l)

    out_path = BASE / f"MAPairs_Adjudication_{len(disputes)}_items.xlsx"
    wb.save(out_path)
