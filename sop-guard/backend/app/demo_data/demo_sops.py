...
DEMO_SOPS = [
    # ------------------------------------------------------------------ 1
    {
        ...
        "raw_text": (
            ...
            "Notify the ICU attending physician immediately if any of the following occur:\n"
            "- MAP remains <65 mmHg after the initial 30 mL/kg fluid bolus and first-line "
            "norepinephrine titration.\n"
            "- Norepinephrine dose exceeds 0.5 mcg/kg/min (per Step 8, this is also the "
            "trigger to add vasopressin as a second vasopressor).\n"
            "- Lactate does not clear by at least 50% on repeat measurement at 2-4 hours.\n"
            "- New-onset organ dysfunction develops (altered mentation, worsening "
            "respiratory status, urine output <0.5 mL/kg/hr for 2 consecutive hours).\n"
            "Escalate to the Rapid Response Team if signs of impending cardiac or "
            "respiratory arrest develop at any point in the resuscitation. Escalate to the "
            "Critical Care Fellow on call for any vasopressor requirement exceeding two "
            "agents (norepinephrine + vasopressin) without hemodynamic improvement.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Bedside RN: qSOFA screening, blood culture collection, timed antibiotic "
            "administration, fluid bolus, and hourly reassessment. ICU attending "
            "physician: vasopressor titration decisions and escalation response. "
            "Critical Care Fellow: second-agent vasopressor escalation and refractory-"
            "shock management. Rapid Response Team: emergent deterioration response.\n\n"
            "9. DOCUMENTATION\n"
            ...
            "initiation in the electronic medical record. Document the time and reason "
            "for any escalation notification, and the name/role of the person notified.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 2
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "Notify the ordering physician immediately for any suspected transfusion "
            "reaction (Step 11 criteria). Escalate to the Rapid Response Team for "
            "anaphylaxis, hypotension unresponsive to stopping the transfusion, or "
            "respiratory distress. Notify the blood bank medical director for any "
            "suspected hemolytic reaction (hemoglobinuria, fever with rigors) so a "
            "type-and-crossmatch recheck can begin immediately. Escalate to the "
            "attending if two-person verification (Step 6) cannot be completed as "
            "specified - do not proceed on a single verifier's judgment.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Ordering physician: orders the product and reviews indication. Bedside "
            "RN (two required): independent identity/product verification, vital sign "
            "monitoring, and first response to a suspected reaction. Blood bank staff: "
            "type and crossmatch, unit release, and reaction workup. Charge nurse: "
            "escalation point if a second qualified verifier is unavailable.\n\n"
            "9. DOCUMENTATION\n"
            ...
            "Document the time and reason for any escalation notification, and the "
        ...
        "structured_json": {
            "steps": [
                ...
                {"step": 8, "action": "Monitor vital signs every 15 minutes for the first hour, then every 30 minutes until completion."},
                {"step": 9, "action": "Each unit of packed red blood cells must be infused within 4 hours of removal from storage."},
    ...
    # ------------------------------------------------------------------ 3
    {
        ...
        "raw_text": (
            ...
            "Stop the procedure and notify the attending immediately for: arterial "
            "puncture (bright red, pulsatile blood), suspected pneumothorax (sudden "
            "desaturation, decreased breath sounds), or inability to confirm venous "
            "placement after 3 attempts - escalate to a more experienced operator or "
            "interventional radiology rather than continuing. Notify the attending "
            "before proceeding if coagulopathy is discovered after the procedure has "
            "started. A chest X-ray showing malposition or pneumothorax must be "
            "reported to the ordering physician immediately, not at routine rounds.\n\n"
            "9. RESPONSIBLE ROLES\n"
            "Inserting physician/APP: obtains consent, performs the procedure, confirms "
            "placement. Bedside RN: assists with positioning and sterile field, monitors "
            "the patient during insertion. Charge nurse/attending: escalation point for "
            "arterial puncture, pneumothorax, or a third failed attempt.\n\n"
            "10. BUNDLE COMPLIANCE\n"
    ...
    # ------------------------------------------------------------------ 4
    {
        ...
        "raw_text": (
            ...
            "8. ESCALATION CRITERIA\n"
            "Notify endocrinology for any Level 3 severe hypoglycemia (Step 4) or "
            "recurrent Level 2 episodes within 24 hours. Activate rapid response for "
            "altered mental status, seizure, or glucose remaining <54 mg/dL after two "
            "treatment cycles. Notify the attending if IV insulin drip glucose exceeds "
            "250 mg/dL for 2 consecutive checks or trends downward faster than "
            "100 mg/dL/hr, either of which requires drip-rate reassessment.\n\n"
            "9. RESPONSIBLE ROLES\n"
            "Bedside RN: POC glucose checks, initial hypoglycemia treatment, and "
            "insulin administration per order. Attending/covering physician: insulin "
            "regimen adjustments and drip-rate changes. Endocrinology: consulted for "
            "Level 3 events or recurrent hypoglycemia.\n\n"
            "10. DOCUMENTATION\n"
            "Document every glucose result, treatment given, and time to recheck. "
            "Document the time and reason for any escalation notification, and the "
            "name/role of the person notified.\n"
        ),
        "structured_json": {
    ...
    # ------------------------------------------------------------------ 5
    {
        ...
        "raw_text": (
            ...
            "8. ESCALATION CRITERIA\n"
            "Notify the prescriber immediately for INR >5.0, any active bleeding on "
            "anticoagulation, or suspected HIT (platelet drop >50% from baseline "
            "4-14 days after heparin exposure). Activate the massive transfusion/"
            "reversal pathway and notify the attending immediately for any "
            "life-threatening bleed (Step 9-12 reversal agents) rather than waiting "
            "for routine pharmacy review. Notify pharmacy for any DOAC ordered in a "
            "patient with a mechanical heart valve before the first dose is given.\n\n"
            "9. RESPONSIBLE ROLES\n"
            "Prescriber: selects agent, sets target INR/aPTT, orders reversal. "
            "Pharmacist: dose verification, renal-function-based DOAC adjustment, "
            "and HIT/interaction screening. Bedside RN: administration, bleeding "
            "precautions education, and first-line bleeding assessment.\n\n"
            "10. DOCUMENTATION\n"
            "Document indication, target INR/aPTT, dose changes, monitoring results, "
            "and patient education on bleeding precautions. Document the time and "
            "reason for any escalation notification, and the name/role of the "
            "person notified.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 6
    {
        ...
        "raw_text": (
            ...
            "8. ESCALATION CRITERIA\n"
            "Report unresolved medication discrepancies to the attending within 2 hours "
            "(see Section 7). Escalate to pharmacy immediately for any high-alert "
            "medication discrepancy (Step 13 list) discovered at admission, transfer, "
            "or discharge - do not wait for the routine verification window. Notify "
            "the prescriber directly, not just the chart, if a chronic medication "
            "(beta-blocker, antiepileptic, antidepressant) appears omitted without "
            "documented rationale.\n\n"
            "9. RESPONSIBLE ROLES\n"
            "Admitting nurse or pharmacist: obtains the BPMH. Pharmacist: high-alert "
            "medication verification and discrepancy resolution. Prescriber: reviews "
            "and approves the reconciled list at each transition of care. Discharging "
            "nurse: teach-back and patient education.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 7
    {
        ...
        "raw_text": (
            ...
            "Notify the physician immediately for any fall with suspected injury, "
            "head strike, or anticoagulant use (bleeding risk). Escalate to the "
            "charge nurse for a repeat fall within 24 hours or when high-risk "
            "interventions (Section 6) are not feasible with current staffing - a "
            "1:1 sitter request should not wait for the next huddle. Any fall with "
            "suspected spinal injury requires activating the facility's spinal "
            "precautions protocol before any patient movement.\n\n"
            "10. RESPONSIBLE ROLES\n"
            "Bedside RN: MFS scoring, intervention implementation, and immediate "
            "post-fall assessment. Charge nurse: staffing escalation (sitter "
            "requests) and post-fall huddle coordination. Physician: post-fall "
            "medical evaluation and imaging orders. Physical therapy: high-risk "
    ...
    # ------------------------------------------------------------------ 8
    {
        ...
        "raw_text": (
            ...
            "Notify infection control immediately for a new or suspected airborne "
            "pathogen (TB, measles, varicella) so AIIR placement and contact tracing "
            "can begin without delay. Notify the charge nurse/bed manager immediately "
            "if no AIIR bed is available (Step 15) so transfer can be arranged. Notify "
            "infection control before downgrading or discontinuing any isolation "
            "precaution - this decision requires ID consultation, not unilateral "
            "nursing judgment.\n\n"
            "7. RESPONSIBLE ROLES\n"
            "Bedside RN: PPE compliance, isolation sign posting, and equipment "
            "dedication. Infection control practitioner: isolation-type determination, "
            "clearance criteria review, and downgrade approval. Environmental "
            "services: terminal cleaning per isolation type.\n"
        ),
        "structured_json": {
            "steps": [
                ...
                {"step": 10, "action": "Eye protection (face shield or goggles) is required for splash-generating procedures."},
    ...
    # ------------------------------------------------------------------ 9
    {
        ...
        "raw_text": (
            ...
            "Activate the cardiac catheterization lab immediately for ROSC with ECG "
            "evidence of STEMI. Consider activating ECMO/extracorporeal CPR for "
            "refractory VF/pVT in a patient with a reversible cause and no ROSC after "
            "standard ACLS, per institutional ECMO activation criteria. Any re-arrest "
            "after ROSC requires immediate re-activation of Code Blue, not a page to "
            "the primary team. Termination decisions (Step 15) require both the team "
            "leader AND the attending physician - neither may decide unilaterally.\n\n"
            "12. RESPONSIBLE ROLES AND ACCOUNTABILITY\n"
            "Team leader (physician or senior resident): directs the resuscitation, "
            "makes the shockable/non-shockable rhythm call, and leads the termination "
            "discussion. Charge nurse: ensures Step 13 role assignments are staffed "
            "and coordinates ICU bed availability for post-ROSC transfer. Attending "
            "physician: joint accountability for the termination decision.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 10
    {
        ...
        "raw_text": (
            ...
            "certain agents).\n\n"
            "8. ESCALATION CRITERIA\n"
            "Activate emergency response/Code Blue immediately for any severe reaction "
            "(Step 10) - do not wait to see if epinephrine works before calling. "
            "Notify the attending radiologist immediately for any moderate reaction "
            "(Step 9) even if it resolves, so the study and future premedication plan "
            "can be reviewed. Notify endocrinology before contrast in any patient with "
            "suspected untreated hyperthyroidism, and nephrology before gadolinium in "
            "any patient with eGFR <30.\n\n"
            "9. RESPONSIBLE ROLES\n"
            "Ordering physician: documents indication and risk factors. Radiologist: "
            "risk stratification review and premedication approval; must be present "
            "for high-risk injections (Step 5). Radiology nurse/technologist: "
            "premedication administration, injection monitoring, and first response "
            "to a reaction.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 11
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "Activate interventional neuroradiology immediately for any large-vessel "
            "occlusion within the thrombectomy window (Step 7) - do not wait for "
            "thrombolysis to finish first. Notify the stroke attending immediately for "
            "post-thrombolysis neurological decline (new NIHSS increase) or blood "
            "pressure exceeding the 24-hour ceiling (Step 10) - either may indicate "
            "hemorrhagic conversion and needs an emergent repeat head CT, not the "
            "scheduled 24-hour one. Escalate to the ICU if door-to-CT or door-to-"
            "needle targets cannot be met due to a competing emergency, so an "
            "alternate imaging or treatment pathway can be arranged.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "ED physician/stroke team: Code Stroke activation, NIHSS, and thrombolysis "
            "decision. Neurology/stroke attending: thrombectomy evaluation and "
            "post-thrombolysis management. Bedside RN: neuro checks per Step 8 "
            "schedule and BP monitoring. Interventional neuroradiology: thrombectomy "
            "for eligible LVO.\n\n"
            "9. DOCUMENTATION\n"
            "Document last-known-well time, NIHSS score, door-to-CT and door-to-needle "
            "times, thrombolysis eligibility decision, and all neuro/BP checks. "
            "Document the time and reason for any escalation notification, and the "
            "name/role of the person notified.\n"
        ),
        "structured_json": {
            "steps": [
                ...
                {"step": 8, "action": "After thrombolysis, perform neuro checks every 15 minutes for 2 hours, then every 30 minutes for 6 hours."},
                {"step": 9, "action": "Hold antiplatelet and anticoagulant therapy for 24 hours after alteplase administration; repeat head CT before resuming."},
    ...
    # ------------------------------------------------------------------ 12
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "Escalate to the House Supervisor per Step 3 whenever no appropriate bed is "
            "available within 90 minutes of the admission order - do not wait for the next "
            "occupancy review cycle. Escalate to the Capacity Command Center immediately on "
            "reaching Red surge (Step 5), not after the next scheduled dashboard review. If "
            "ED boarding exceeds 4 hours (Step 6) and no bed has been prioritized, the House "
            "Supervisor must escalate to the on-call Administrator for a cross-unit override "
            "decision.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Bed Management Center: occupancy monitoring and initial bed assignment. House "
            "Supervisor: surge-level escalation and cross-unit override decisions. Receiving "
            "unit charge nurse: bed-readiness confirmation before transport. On-call "
            "Administrator: Capacity Command Center activation and diversion decisions.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 13
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "If a high-risk patient (Step 3 flag) reaches the anticipated discharge day "
            "without a confirmed post-discharge care plan or named caregiver/facility "
            "contact, the case manager must escalate to the attending physician and unit "
            "charge nurse before a discharge order is written. If medication reconciliation "
            "(Step 5) cannot be completed by pharmacy within 24 hours of anticipated "
            "discharge for a patient on 5+ chronic medications, escalate to the "
            "pharmacist-in-charge to prioritize the case. Escalate unresolved discharge "
            "barriers identified on Step 4 rounds to the unit's daily flow huddle if not "
            "resolved within 48 hours.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Case manager/social worker: risk screening, care-plan coordination, and "
            "escalation of unresolved barriers. Bedside RN: discharge education and warm "
            "handoff call (Step 8). Pharmacist: medication reconciliation for high-"
            "complexity regimens. Attending physician: discharge order and high-risk "
            "care-plan sign-off.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 14
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "Escalate to the supervising attending immediately if a trainee/resident note "
            "is not co-signed within the 24-hour window (Step 3). Escalate any chart "
            "auto-flagged for copy-forward review (Step 8, >10% verbatim match) to the "
            "Compliance office within 5 business days, and to the provider's department "
            "chair if a second flag occurs within 90 days. Escalate any discovered instance "
            "of an entry being deleted or overwritten outside the formal amendment workflow "
            "(Step 7) to Compliance immediately as a documentation-integrity incident.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Documenting clinician: timely, accurate entries and formal amendments only. "
            "Supervising attending: trainee note co-signature within 24 hours. Compliance "
            "office: copy-forward flag review and documentation-integrity incident "
            "handling. Department chair: repeat-offender escalation.\n"
        ),
        "structured_json": {
            "steps": [
                ...
                {"step": 6, "action": "Complete discharge summaries within 48 hours of discharge, or within 24 hours if the patient has a follow-up appointment scheduled sooner."},
    ...
    # ------------------------------------------------------------------ 15
    {
        ...
        "raw_text": (
            ...
            "reporting cycle.\n\n"
            "7. ESCALATION CRITERIA\n"
            "Escalate any Critical finding (Step 5 classification) to the Compliance "
            "Committee and Chief Medical Officer immediately upon identification, per "
            "Step 9 - do not batch it into the preliminary findings report. If a department "
            "fails to submit a CAPA within the 10-business-day window (Step 7), escalate to "
            "the department's executive sponsor. If a follow-up audit (Step 8) finds the "
            "same Critical or Major issue unresolved, escalate to the Compliance Committee "
            "as a repeat-finding failure requiring executive review.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Compliance Office: audit scoping, sampling, scoring, and findings reporting. "
            "Department leadership: CAPA submission and remediation. Compliance Committee "
            "and Chief Medical Officer: immediate review of escalated Critical findings and "
            "repeat-finding failures.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 16
    {
        ...
        "raw_text": (
            ...
            "7. ESCALATION CRITERIA\n"
            "Immediately suspend and escalate to the Credentials Committee chair and the "
            "candidate's department chief if a license lapse, exclusion-list match, or "
            "unresolved malpractice claim is identified at any point (Step 8) - this "
            "overrides the normal reappointment-cycle timeline. If primary source "
            "verification (Step 1) cannot be completed before the candidate's scheduled "
            "start date, escalate to HR leadership to delay the start date rather than "
            "proceed without verification. Escalate any preceptor sign-off gap at the "
            "4-week mark (Step 5) to the department chief before independent practice "
            "begins.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "HR credentialing staff: primary source verification and background screening. "
            "Preceptor: supervised-period sign-off. Credentials Committee: privileging "
            "review and reappointment decisions. Department chief: immediate response to "
            "suspended-privilege escalations.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 17
    {
        ...
        "raw_text": (
            ...
            "completion.\n\n"
            "7. ESCALATION CRITERIA\n"
            "Escalate to the department manager when an employee's core competency "
            "training reaches the 15-day-overdue EHR-suspension trigger (Step 4), so the "
            "manager can arrange coverage before access is cut off. Escalate any "
            "department reported below the 90% compliance flag threshold (Step 8) to the "
            "Quality Committee for a corrective action plan. Escalate a request to restore "
            "suspended EHR access to HR only after training completion is confirmed - "
            "never on department-manager attestation alone.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "HR training administration: module assignment, reminders, and record "
            "retention. Department managers: monthly compliance follow-up and staffing "
            "coverage around suspensions. Quality Committee: quarterly compliance-rate "
            "review and corrective action planning for flagged departments.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 18
    {
        ...
        "raw_text": (
            ...
            "action.\n\n"
            "7. ESCALATION CRITERIA\n"
            "For any Sentinel Event, escalate to the House Supervisor and Risk Management "
            "immediately by phone within 1 hour (Step 2) - the electronic report alone is "
            "not sufficient notification. Escalate any Severe or Sentinel-severity triage "
            "result (Step 4) to convene the RCA team within 5 business days (Step 5); a "
            "missed RCA convening deadline must be escalated to the Patient Safety "
            "Committee chair. Escalate any corrective action not verified effective at the "
            "90-day check (Step 8) back to the original RCA team for re-evaluation.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Reporting staff: timely event submission and evidence preservation. Risk "
            "Management: triage, severity scoring, and Sentinel Event notification. RCA "
            "team: root cause analysis and corrective action tracking. Patient Safety "
            "Committee: RCA findings review and escalation of missed deadlines.\n"
        ),
        "structured_json": {
            "steps": [
    ...
    # ------------------------------------------------------------------ 19
    {
        ...
        "raw_text": (
            ...
            "sustainment check.\n\n"
            "7. ESCALATION CRITERIA\n"
            "If a PDSA test cycle (Step 4) shows no meaningful improvement after two "
            "consecutive cycles (Step 6), escalate the abandon/redesign decision to the "
            "Quality Committee rather than letting the project owner decide unilaterally. "
            "Escalate any project that misses two consecutive monthly status reports "
            "(Step 7) to the Quality Committee chair for a project-viability review. "
            "Escalate a failed post-adoption sustainment check (Step 8) - metric regressing "
            "after house-wide rollout - back to the Quality Committee as a new improvement "
            "opportunity, not a closed project.\n\n"
            "8. RESPONSIBLE ROLES\n"
            "Project owner/department lead: PDSA cycle execution and monthly status "
            "reporting. Quality Committee: project sponsorship, viability review, and "
            "abandon/expand decisions. Quality data analyst: baseline and sustainment-"
            "check measurement.\n"
        ),
        "structured_json": {
            "steps": [
